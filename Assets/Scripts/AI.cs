...
public class AI : MonoBehaviour {
	...
	TileManager.Tile GetTargetTilePerGhost()
	{
		...
		case "covidr_1":	// target = pacman + 4*pacman's direction (4 steps ahead of pacman)
		...
		// 	targetTile = tiles[manager.Index((int)targetPos.x, (int)targetPos.y)];
		// 	break;
		case "covidr_3":	// target = ambushVector(pacman+2 - blinky) added to pacman+2
			targetPos = new Vector3 (target.position.x+0.499f, target.position.y+0.499f);
			targetTile = tiles[manager.Index((int)targetPos.x, (int)targetPos.y)];
		...
		// 		targetTile = tiles[manager.Index (0, 2)];
		// 	break;
		case "covidb":	// target = pacman
			targetPos = new Vector3 (target.position.x+0.499f, target.position.y+0.499f);
			targetTile = tiles[manager.Index((int)targetPos.x, (int)targetPos.y)];
			break;
		case "covidb_1":	// target = pacman + 4*pacman's direction (4 steps ahead of pacman)
			dir = target.GetComponent<PlayerController>().getDir();
			targetPos = new Vector3 (target.position.x+0.499f, target.position.y+0.499f) + 4*dir;
		...
		// 	targetTile = tiles[manager.Index((int)targetPos.x, (int)targetPos.y)];
		// 	break;
		case "covidb_2":	// target = pacman
			dir = target.GetComponent<PlayerController>().getDir();
			//Vector3 blinkyPos = GameObject.Find ("blinky").transform.position;
			Vector3 ambushVector1 = target.position + 2*dir; //- blinkyPos ;
			targetPos = new Vector3 (target.position.x+0.499f, target.position.y+0.499f) + 2*dir + ambushVector1;
			targetTile = tiles[manager.Index((int)targetPos.x, (int)targetPos.y)];
			break;
		// case "inky":	// target = ambushVector(pacman+2 - blinky) added to pacman+2
		// 	dir = target.GetComponent<PlayerController>().getDir();
		...
		// 	targetTile = tiles[manager.Index((int)targetPos.x, (int)targetPos.y)];
		// 	break;
		case "covidb_3":	// target = ambushVector(pacman+2 - blinky) added to pacman+2
			targetPos = new Vector3 (target.position.x+0.499f, target.position.y+0.499f);
			targetTile = tiles[manager.Index((int)targetPos.x, (int)targetPos.y)];
			// if(manager.distance(targetTile, currentTile) < 9)
			// 	targetTile = tiles[manager.Index (0, 2)];
			break;
		case "covidc":	// target = pacman
			targetPos = new Vector3 (target.position.x+0.499f, target.position.y+0.499f);
			targetTile = tiles[manager.Index((int)targetPos.x, (int)targetPos.y)];
			break;
		case "covidc_1":	// target = pacman + 4*pacman's direction (4 steps ahead of pacman)
			dir = target.GetComponent<PlayerController>().getDir();
			targetPos = new Vector3 (target.position.x+0.499f, target.position.y+0.499f) + 4*dir;
			...
			targetTile = tiles[manager.Index((int)targetPos.x, (int)targetPos.y)];
			break;
		case "covidc_2":	// target = pacman
			dir = target.GetComponent<PlayerController>().getDir();
			//Vector3 blinkyPos = GameObject.Find ("blinky").transform.position;
			Vector3 ambushVector2 = target.position + 2*dir; //- blinkyPos ;
			targetPos = new Vector3 (target.position.x+0.499f, target.position.y+0.499f) + 2*dir + ambushVector2;
			targetTile = tiles[manager.Index((int)targetPos.x, (int)targetPos.y)];
			break;
		
		case "covidc_3":	// target = ambushVector(pacman+2 - blinky) added to pacman+2
			targetPos = new Vector3 (target.position.x+0.499f, target.position.y+0.499f);
			targetTile = tiles[manager.Index((int)targetPos.x, (int)targetPos.y)];
			// if(manager.distance(targetTile, currentTile) < 9)
			// 	targetTile = tiles[manager.Index (0, 2)];
